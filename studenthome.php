...
<tr  text-align = "center"> <td colspan="3"> 
<form name="buttons">
		...
		<?php 
			...
			// $row = mysqli_fetch_assoc($result);
				if(mysqli_num_rows($result) >0) {
					...
					echo '<table border="1"  style="text-align:center;background-color:pink; margin-left:55rem;">';
					echo "<tr><th>Lecturer ID </th><th>Lecturer Name</th><th>Field/Expertise</th><th>%</th>";
					while($rowLec = mysqli_fetch_assoc($result1)) {
						...
						$smallPerc = ceil($perc);		
						echo "<tr><td>".$rowLec['LecNo']."</td><td>".$rowLec['Name']."</td><td>".$rowLec['Expertise']."</td><td>".$smallPerc."</td></tr>";
							
					} 
					echo "</table><br /><br /><br /><br /><hr />";
				} else {
					echo "0 results: You have a supervisor";
				}
		?>

	</div>
	<div>
	<a href="select.php">select a lecturer</a>
	<<?php 
	require('connection.php');
	echo "Haha";
		$lec = "SELECT Name FROM lecturer WHERE LecNo = 22";
		$results = mysqli_query($conn, $lec) or die (mysqli_error());
		$output = mysqli_fetch_array($results);
		if ($conn->query($lec) === TRUE) {
 	 	echo "Lecturer Selected";
			header('refresh:5; url="studenthome.php"');
		} else {
		  echo "Error updating record: " . $conn->error;
		echo "Your Supervisor is: ";
	}
	 ?>
	</div>
</td></tr> 
...
<td width="200px">
<div id="social">
	<u style="margin-bottom: 20px;">FOLLOW US</u><br />
	<a href="#"  >Twitter</a><br />
...
<div style="background-color: #348017; width: 100%; height: 70px; display: flex; justify-content: center; align-items: center;">
